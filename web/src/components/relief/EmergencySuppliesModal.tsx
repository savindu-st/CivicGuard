  ...
  return (
    <div className="fixed inset-0 z-[2000] bg-dark-950/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-dark-900 border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* Tab Toggle Bar */}
        <div className="flex border-b border-slate-800 bg-dark-950/50 px-6 pt-2">
          <button
          ...
          {activeTab === 'ALLOCATE' ? (
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-300">Target SOS Help Request:</label>
                <select
                  value={selectedRequestId}
                  onChange={(e) => setSelectedRequestId(e.target.value)}
                  className="w-full bg-dark-950 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-brand-400"
                >
                ...
                {currentSOS && (
                  <div className="p-2.5 rounded-lg bg-dark-850/90 border border-slate-800 text-[11px] text-slate-300 flex items-center justify-between">
                    <span>
                      ...
                      return (
                        <div
                          ...
                          className={`p-3 rounded-xl border flex items-center justify-between gap-3 transition-all ${
                            currentQty > 0
                              ? 'bg-dark-800 border-emerald-500/80 shadow-sm'
                              : 'bg-dark-850/60 border-slate-800'
                          }`}
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className="text-xs font-bold text-white">{res.resource_name}</h4>
                              <p className="text-[11px] text-slate-400">
                                Center: {res.shelter_name || 'Relief Warehouse'} • Stock:{' '}
                                <strong className="text-emerald-400">{res.quantity} {res.unit}</strong>
                              </p>
                            </div>
                          </div>

                          {/* Quantity Counter Control */}
                          <div className="flex items-center gap-2 bg-dark-950 px-2 py-1 rounded-lg border border-slate-700">
                            <button
                              type="button"
                            ...
                            <input
                              ...
                              className="w-10 text-center bg-transparent font-bold text-xs text-white focus:outline-none"
                            />
                            <button
                              type="button"
                              onClick={() => handleQuantityChange(res.id, res.quantity, currentQty + 1)}
            ...
            <form onSubmit={handleRestockSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-300">Receiving Center / Shelter:</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-300">Resource Category:</label>
                  <select
                    value={restockType}
                    onChange={(e) => setRestockType(e.target.value)}
                    className="w-full bg-dark-950 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white focus:outline-none"
                  >
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-300">Unit of Measurement:</label>
                  <select
                    value={restockUnit}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2 space-y-1.5">
                  <label className="text-xs font-bold text-slate-300">Supply Item Name:</label>
                  <input
                    type="text"
                    required
                    value={restockName}
                    ...
                    className="w-full bg-dark-950 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none"
                  />
                </div>
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-brand-600 hover:bg-brand-500 text-white font-bold text-xs tracking-wide shadow-lg shadow-brand-600/30 flex items-center justify-center gap-2 transition-all border border-brand-400"
              >
                {isSubmitting ? (
          ...
          <div className="px-6 py-4 border-t border-slate-800 bg-dark-850 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs tracking-wide shadow-lg shadow-emerald-600/30 flex items-center gap-2 transition-all border border-emerald-400"
              >
                {isSubmitting ? (
                  <>
